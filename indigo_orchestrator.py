...
class powermanager(PowerManager):
    ...
    def _modify_deployment(self, vms, remove_node=None, add_node=None):
        inf_id = self._get_inf_id()
        ...
        template = self._get_template(len(vms), remove_node, add_node)
    ...
    def power_on(self, nname):
        try:
            ...
            resp_status, output = self._modify_deployment(vms, add_node=nname)
    ...
    def _power_off(self, nname, vmid):
        try:
            ...
            resp_status, output = self._modify_deployment(self._mvs_seen, remove_node=str(vmid))
    ...
    def _get_template(self, count, remove_node, add_node):
        inf_id = self._get_inf_id()
        ...
        if resp.status != 200:
            ...
            if remove_node:
                ...
                node_template['capabilities']['scalable']['properties']['removal_list'] = [remove_node]
            else:
                node_template['capabilities']['scalable']['properties']['count'] = count + 1
                # Put the dns name
                ...
                node_template['capabilities']['endpoint']['properties']['dns_name'] = add_node
