...
class TestCondorPlugin(unittest.TestCase):
	...
	def test_infer_chronos_state_idle(self):
		assert cluesplugins.condor.infer_clues_job_state(1) == Request.PENDING

	def test_infer_chronos_state_running(self):
		assert cluesplugins.condor.infer_clues_job_state(2) == Request.ATTENDED

	def test_infer_chronos_state_removed(self):
		assert cluesplugins.condor.infer_clues_job_state(3) == Request.ATTENDED

	def test_infer_chronos_state_completed(self):
		assert cluesplugins.condor.infer_clues_job_state(4) == Request.ATTENDED

	def test_infer_chronos_state_held(self):
		assert cluesplugins.condor.infer_clues_job_state(5) == Request.ATTENDED
        
	def test_infer_chronos_state_transferout(self):
		assert cluesplugins.condor.infer_clues_job_state(6) == Request.ATTENDED

	def test_infer_chronos_state_suspended(self):
		assert cluesplugins.condor.infer_clues_job_state(7) == Request.ATTENDED
		
	def test_init_lrms_empty(self):
		lrms = cluesplugins.condor.lrms()
		assert lrms._server_ip == 'htcondoreserver'

	def test_init_lrms(self):
		lrms = cluesplugins.condor.lrms('test_ip')
	...
	def test_get_jobinfolist(self,locateAll,query):
		...
		job_info_list = cluesplugins.condor.lrms(MagicMock(cluesplugins.condor.lrms)).get_jobinfolist()
		lenj = len(job_info_list)
		assert lenj == test_numjobs


	@mock.patch('htcondor.Schedd.query')
	@mock.patch('cluesplugins.condor.get_schedulers_list_from_Schedd')
	@mock.patch('cluesplugins.condor.get_worker_nodes_list_from_Startd')
	def test_get_nodeinfolist(self,get_worker_nodes_list,get_schedulers_list,query):
		...
		node_info_list = cluesplugins.condor.lrms(MagicMock(cluesplugins.condor.lrms)).get_nodeinfolist()
		if node_info_list:
